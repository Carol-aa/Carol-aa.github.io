...
Snake.prototype.strategies = {
	move: function (format) {
		//这个参数用于决定要不要删除最后一个方块（蛇尾）。当传了这个参数后就表示要做的事情是吃
		//创建新身体（在旧蛇头的位置）
	...
	eat: function () {
		...
		var preScore = game.score;

		$('.score1').text(preScore);
...
function createFood() {
	...
	var y = null;
	var include = true;
	//循环跳出的条件，true表示食物的坐标在蛇身上（需要继续循环）。false表示食物的坐标不在蛇身上（不循环了）
	while (include) {
...
Game.prototype.over = function () {
	var music = document.querySelector('#music');//获取音乐
...
var tem = false;
var temp = false;
var sound0 = document.querySelector('.sound0');
game = new Game();
